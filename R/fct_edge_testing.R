...
#'
#' @examples
test_edges <- function(mat, nGenes, nRegulators, density = 0.02, nTrees = 1000, 
                       nShuffle = 1000, 
  ...
  if(fdr <= 0 | fdr > 1)
    stop("fdr threshold must be strictly positive between 0 and 1, standard values being 0.01, 0.05, or O.1")
  ...
  if(verbose)
    message(paste("First network thresholding : Selecting", nEdges, "edges for a gloabl density of", density, "."))
  ...
  suppressPackageStartupMessages(result.reg <- 
                                   doRNG::"%dorng%"(foreach::foreach(target=targets, .combine=rbind),
                                                    {
                                                      # remove target gene from input genes
                                                      theseRegulatorNames <- target_to_TF[[target]]
                                                      numRegulators <- length(theseRegulatorNames)
                                                      
                                                      y <- dataT[, target]
                                                      
                                                      x <- data.frame(dataT[, target_to_TF[[target]]])
                                                      colnames( x ) <- target_to_TF[[target]]
                                                      ...
                                                      if(length(target_to_TF[[target]]) == 1){
                                                        pval <- rf$pval[, , "scaled"]
                                                        names(pval) <- paste(names(pval), ".pval", sep = "")
                                                        
                                                        pvals <- setNames(pval["%IncMSE.pval"], target_to_TF[[target]])
                                                      }
                                                      else{
                                                      ...
                                                      res <- data.frame("regulatoryGene" = theseRegulatorNames, "targetGene" = rep(target, numRegulators))
                                                      res$pval <- pvals[match(res$regulatoryGene, names(pvals))]
                                                      res
                                                    }))
  ...
  distr <- ggplot2::ggplot(d, ggplot2::aes( x = value, fill = variable)) + 
    ggplot2::geom_density(alpha = 0.5) + ggplot2::xlim(0,0.5)
  
  nedges <- sapply(X = seq(0.01, 0.1, by = 0.001), FUN = function(fdr){
    return(sum(links$fdr < fdr))
  })
  d <- data.frame(FDRs = seq(0.01, 0.1, by = 0.001), n_edges = nedges)
  curve <- ggplot2::ggplot(d, ggplot2::aes(x = FDRs, y = n_edges)) + 
    ggplot2::geom_point(size = 3, color = "darkgreen") + 
    ggplot2::ggtitle("Number of edges depending on the FDR threshold") + 
    ggplot2::xlab("fdr")
  
  return(list(links = links, fdr_nEdges_curve = curve, pvalues_distributions = 
                distr))
}
...
network_from_tests <- function(links, fdr){
  ...
  if (fdr <= 0 | fdr > 1)
    stop("fdr threshold must be strictly positive between 0 and 1, 
